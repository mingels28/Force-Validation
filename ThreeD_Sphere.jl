...
""" geometry function """
function sphere(p=8;Re=1000,U=1,mem=Array,ϵ=0.1)
    n = 2^p                     #number of grid points
    L = n/20                    #characteristic geometry size
    center3 = SA[n/2,n/2,n/4]   #center of 3D domain
    ...
    #3D simulation
    WaterLily.Simulation((2n,n,Int(n/2)),(U,0,0),L;U,ν=U*L/Re,
        body=WaterLily.AutoBody(sdf),mem=mem,T=Float32,ϵ=ϵ) 
end
...
function get_force(sim,t)
	...
	return -WaterLily.∮nds(sim.flow.p,df,sim.body,t*sim.L/sim.U)./(0.5*sim.L^2*sim.U^2)
end
...
sim  = sphere(p;Re=Re,mem=CUDA.CuArray); 
